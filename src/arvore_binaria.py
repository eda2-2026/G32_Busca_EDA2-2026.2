...
class ArvoreBinariaBusca:
    ...
    def exibir_arvore_grafica(self):
        print("\nEstrutura Visual da Árvore Binária (Inclinada):")
        if self.raiz is None:
            print("Árvore vazia.")
        else:
            self._exibir_arvore_grafica(self.raiz, 0)

    def _exibir_arvore_grafica(self, no, nivel):
        if no is not None:
            self._exibir_arvore_grafica(no.direita, nivel + 1)
            print("      " * nivel + f"├── {no.veiculo.placa}")
            self._exibir_arvore_grafica(no.esquerda, nivel + 1)